...
export class MessageService {
  ...
  async create(createMessageDto: CreateMessageDto) {
    ...
    this.socketService.socket.to(createMessageDto.dialogId.toString()).emit("message:created", message)
    return message
  }
  ...
  async findAllByDialog(userId: number, dialogId: number) {
    const a = await this.repository.find({relations: ['dialog', 'user'], where: {user: {id: userId}}})
    console.log(a);

    
    this.socketService.socket.to(dialogId.toString()).emit("messages:readed", dialogId.toString(), userId)
    return a
  }
